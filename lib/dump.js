...
`);
  ...
  const { format = 'yaml', directory } = config;
  ...
  const stats = await mapAsync(locales, async (locale) => {
    ...
    const contentStats = await mapAsync(entries, async (entry) => {
      ...
      if (mapped) {
        const content = await convert(mapped, format);
        const localeAddon = locale.default ? '' : `@${locale.code}`;
        const filename = `${id}${localeAddon}.${format}`;
        const filepath = path.join(directory, getContentTypeId(entry), filename);
...
// module.exports.getFieldSettings = getFieldSettings;
