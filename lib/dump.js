...
`);
  ...
  const { format = 'yaml', directory, mapDirectory, mapFilename, transform } = config;
  ...
  const stats = await mapAsync(locales, async (locale) => {
    ...
    const contentStats = await mapAsync(entries, async (entry) => {
      ...
      if (mapped) {
        ...
        let data = Object.fromEntries(Object.entries(mapped).map(([key, value]) => [snakeCase(key), value]));
        ...
        const dir =
          typeof mapDirectory === 'function'
            ? await mapDirectory(directory, { locale, contentType, entry, format })
            : path.join(directory, contentType, filename);
