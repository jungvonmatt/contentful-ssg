 ...
 */
const mapMetaFields = (entry, options) => {
  const { sys } = entry;
...
const mapEntry = async (entry, options) => {
  const { fieldSettings, validate, mapMetaFields: customMetaFields } = options;
  const contentType = getContentTypeId(entry);
  const { [contentType]: settings } = fieldSettings;
  const { sys, fields } = entry;
  const { id } = sys || {};

  const mapMetaFieldsFunction = typeof customMetaFields === 'function' ? customMetaFields : mapMetaFields;
  const meta = mapMetaFieldsFunction(entry, { contentType, ...options });
  ...
  const data = await mapAsync(Object.entries(fields), async ([key, content]) => {
    const value = await mapField(content, { ...options, settings: settings[key] || {}, fieldId: key });
