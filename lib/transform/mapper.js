 ...
 * @param {*} settings field settings from contentful content-type
 */
...
const mapEntry = async (entry, options) => {
  const { entries, assets, contentTypes } = options;
  ...
  const data = await mapAsync(Object.entries(fields), async ([key, content]) => {
    const value = await mapField(content, { settings: fieldSettings[key] || {}, assets, entries });
    return [key, value];
  });
