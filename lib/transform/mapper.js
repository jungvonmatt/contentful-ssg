...
const mapReferenceField = (fieldContent, options) => {
  ...
  if (isEntryLink(fieldContent)) {
    ...
    return typeof customMapEntryLink === 'function' ? customMapEntryLink(entry) : mapEntryLink(entry);
  }

  if (isEntry(fieldContent)) {
    return typeof customMapEntryLink === 'function' ? customMapEntryLink(fieldContent) : mapEntryLink(fieldContent);
  }
