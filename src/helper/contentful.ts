...
import type {ContentfulConfig, FieldSettings, Node, Entry, Asset, ContentType, Locale} from '../types.js';
 ...
 * @returns {String}
 */
export const getContentTypeId = <T extends Node | EntryFields.Link<unknown>>(node: T): string => node?.sys?.contentType?.sys?.id ?? 'unknown';
 ...
 * @returns {String}
 */
export const getEnvironmentId = <T extends Node>(node: T): string => node?.sys?.environment?.sys?.id ?? 'unknown';
 ...
 * @returns {String}
 */
export const getContentId = <T extends Node | ContentType | EntryFields.Link<unknown>>(node: T): string => node?.sys?.id ?? 'unknown';
 ...
 * - getAssets
 */
const pagedGet = async <T>(apiClient, {method, skip = 0, aggregatedResponse = null, query = null}: PagedGetOptions<T>): Promise<CollectionProp<T>> => {
 ...
 */
export const convertToMap = <T extends Node>(nodes: T[] = []) => new Map(nodes.map(node => [getContentId(node), node]));
